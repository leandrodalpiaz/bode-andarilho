...
async def finalizar_cadastro_evento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not grupo_id or grupo_id in VALORES_NAO_INFORMADO:
        ...
        try:
            ...
            data_clean = dados_evento['data'].replace('/', '_')
            nome_clean = re.sub(r'[^a-zA-Z0-9]', '_', dados_evento['nome_loja'])
            numero_clean = re.sub(r'[^a-zA-Z0-9]', '_', dados_evento['numero_loja'])
            ...
            if "pago" in agape_texto or "dividido" in agape_texto:
                botoes = [
                    [InlineKeyboardButton("🍽 Participar com ágape (pago)", callback_data=f"confirmar_{evento_id}_pago")],
                    [InlineKeyboardButton("🚫 Participar sem ágape", callback_data=f"confirmar_{evento_id}_sem")],
                ]
            elif "gratuito" in agape_texto:
                botoes = [
                    [InlineKeyboardButton("🍽 Participar com ágape (gratuito)", callback_data=f"confirmar_{evento_id}_gratuito")],
                    [InlineKeyboardButton("🚫 Participar sem ágape", callback_data=f"confirmar_{evento_id}_sem")],
                ]
            else:  # sem ágape
                botoes = [
                    [InlineKeyboardButton("✅ Confirmar presença", callback_data=f"confirmar_{evento_id}_sem")],
                ]
            # Botão "Ver confirmados" sempre presente
            botoes.append([InlineKeyboardButton("👥 Ver confirmados", callback_data=f"ver_confirmados_{evento_id}")])
            ...
            print(f"✅ Evento publicado no grupo {grupo_id_int} com identificador {evento_id}")
